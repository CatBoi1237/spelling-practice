...
import RaceEndgame from "@/components/RaceEndgame";
import {
  buildCustomRacePath,
...
export default function RoomShell() {
  ...
  const inviteUrl = customList
    ? `${window.location.origin}${buildCustomRacePath(roomCode, customList)}`
    : null;
  ...
  const shareInvite = async () => {
    if (!inviteUrl || !customList) return;
    try {
      if (navigator.share) {
  ...
  return (
    <div className={`race-shell space-y-5 ${customList ? "custom-race-shell" : ""}`}>
      <RaceEndgame />
